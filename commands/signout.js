...
// Check if interaction was already replied to or deferred
      ...
      if (!activeSession) {
        await interaction.reply({
          content: "❌ You don't have an active session in the database. Use `/signin` to start a new session.",
          ephemeral: true
        });
        return;
      }
      ...
      const workSummaryInput = new TextInputBuilder()
        ...
        .setPlaceholder('Summarize your work. You can use bullet points and multiple paragraphs.')
        .setRequired(true)
        .setMinLength(10)
        .setMaxLength(4000);
        ...
        .addFields(
          {
            ...
            value: `\`\`\`
Started: ${new Date(sessionData.StartTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })} | ${new Date(sessionData.StartTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
Break Time: ${breakDurationString}
End: ${timeString} | ${dateString}
Duration: ${workDurationString}
\`\`\``,
            inline: false
          }
        )
